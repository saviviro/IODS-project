...
$$
\text{logit}(Pr(\text{high_use}))=\beta_1\text{age} + \beta_2\text{absences} + \beta_3\text{G3} + \beta_4 \boldsymbol{1}\lbrace \text{goout}=1 \rbrace + \beta_5 \boldsymbol{1}\lbrace \text{goout}=2 \rbrace + \beta_6 \boldsymbol{1}\lbrace \text{goout}=3 \rbrace + \beta_7 \boldsymbol{1}\lbrace \text{goout}=4 \rbrace + \beta_8 \boldsymbol{1}\lbrace \text{goout}=5 \rbrace
$$
...
To get the coefficient estimates for odds (i.e. taking exponent from our model equation which induces an exponential multiplicative relationship between the odds and the explanatory variables), exponent of the estimates needs to be taken. Due to the exponential multiplicative nature of the model for odds, exponents of the coefficients are then also the coefficients for the model for the odds-ratio. In the odds-ratio model, the interpretation of the coefficients is just for the difference between the values of the explanatory values in the two groups and not for the values themselves. Below, we calculate the exponents of the coefficients of the log-odds model (and thus obtain the coefficients for both, the odds model and the log-odds model) and calculate confidence intervals for the exponents coefficients.  
```{r, message=FALSE}
